...
def minibatch_parse(sentences, model, batch_size):
    ...
    def get_batch():
        ...
        i = 0
        while True:
            if len(unfinished_parses[i].stack) != 1 or \
               len(unfinished_parses[i].buffer) != 0:
               batch.append(unfinished_parses[i])
            i += 1
            if len(batch) == batch_size or i == len(unfinished_parses):
                break
        return batch
    ...
    while len(batch) > 0:
        ...
        batch = get_batch()
    dependencies = [parse.dependencies for parse in unfinished_parses]
    return dependencies
    ### END YOUR CODE
